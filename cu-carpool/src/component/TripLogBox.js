...
const TripLogBox = ({ history, data }) => {
  ...
  const fetchData = async () => {
    const response = await axios.get(
      backend + "/admin/trip-request?id=" + trip_id
    );
  ...
  return (
    ...
    >
      <Paper
        ...
        elevation={0}
        className={classes.paper}
        style={{ padding: 3, backgroundColor: "#C78899" }}
      >
      ...
      >
        <div
          ...
          style={{ alignItems: "center", width: "150px", marginRight: "16px" }}
        >
          <img
            src={photo}
            height={100}
        ...
        >
          ...
          <div>
            {" "}
            Driver's Name: {firstname} {lastname}
          </div>
      ...
      <ExpansionPanel square onClick={handleClick} className={classes.root}>
        <ExpansionPanelSummary
          ...
          expandIcon={
            <ExpandMoreIcon fontSize="large" style={{ color: "grey" }} />
          }
        ></ExpansionPanelSummary>
        <ExpansionPanelDetails className={classes.details}>
          <div className={classes.request}>
            {(state.list.length == 0 || false) && (
              <div style={{ margin: 20, color: "#BDBDBD" }}>
                no trip request
              </div>
            )}
