...
class AdminWithdrawal extends React.Component {
  ...
  fetchData = async () => {
    const response = await axios.get(
      "http://localhost:4000/admin/withdrawal/request"
    );
  ...
  handleApprove = async id => {
    ...
    const response = await axios.post(
      "http://localhost:4000/admin/withdrawal-approve",
      { id, admin_name }
    );
  ...
  handleReject = async id => {
    const { username: admin_name } = this.props.user;
    const response = await axios.post(
      "http://localhost:4000/admin/withdrawal-reject",
      { id, admin_name }
    );
    const { success } = response.data;
    if (success) {
          ...
          {this.state.list.map(
            ({
              ...
              >
                ...
                >
                  {balance < amount && (
                    <MyGreyButton disabled>Approve</MyGreyButton>
                  )}
                  {!(balance < amount) && (
                    // <MyButton
                  ...
                  {/* <MyWhiteButton onClick={() => this.handleReject(id)}>
                    Reject
                  </MyWhiteButton> */}
