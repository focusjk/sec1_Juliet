...
router.post('/withdrawal-approve', function (req, res, next) {
	...
	const approved_at = util.timeformatter(new Date());
	const action = 0;
	withdrawalService.withdrawalAction(admin_name, approved_at, id, action, (err, result) => {
		if (err) {
			res.json({
				success: false,
				error: err.sqlMessage,
...
router.post('/withdrawal-reject', function (req, res, next) {
	...
	const rejected_at = util.timeformatter(new Date());
	const action = 1;
	withdrawalService.withdrawalAction(admin_name, rejected_at, id, action, (err, result) => {
		if (err) {
			res.json({
				success: false,
				error: err.sqlMessage,
