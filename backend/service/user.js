...
const getMemberInfo = (username, callback) => {
  return db.query(`SELECT id,username,firstname,lastname,phone_number,email,photo,driver_status,
      amount,driving_license,approved_at,approved_by,rejected_at,rejected_by,edited_at FROM members WHERE username = ?`, [username], callback); //credit-card-related fields are removed from this line
}

function login(username, password, callback) {
  return db.query(`SELECT id,username,firstname,lastname,phone_number,email,photo,driver_status,
    amount,driving_license,approved_at,approved_by,rejected_at,rejected_by,edited_at FROM members WHERE username = ? AND password = ?`, [username, password], callback); //credit-card-related fields are removed from this line
