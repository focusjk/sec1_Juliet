...
const createRequest = async (
  ...
  const check = await util.promisifyQuery(
    `SELECT count(*) as count FROM request WHERE request.trip_id = ? AND request.member_id = ? AND request.request_status not in ('canceled','rejected')`,
    [trip_id, member_id]
  );
  const { count } = check[0];
  ...
  if (count == 0 && owner != member_id) {
    ...
    return db.query(
      `INSERT INTO request (member_id,trip_id,departure_latitude,departure_longitude,departure_detail,destination_latitude,destination_longitude,destination_detail,request_status,created_at) 
        values (?,?,?,?,?,?,?,?,?,?)`,
      [
        member_id,
        trip_id,
...
function getRequestInfo(request_id, callback) {
  return db.query(
    `SELECT id,member_id,trip_id,departure_latitude,departure_longitude,departure_detail,destination_latitude,destination_longitude,destination_detail,
...
const getInTheCar = (request_id, depart_time, callback) => {
  ...
  return db.query(
    `UPDATE request
                     ...
                     WHERE id = ?`,
    [req_status, depart_time, request_id],
    callback
  );
};

const cancelRequest = async (request_id, cancel_time, callback) => {
  const query_result = await util.promisifyQuery(
    `SELECT request_status FROM request WHERE id = ?`,
    [request_id]
  );
  ...
  if (request_status == "pending" || request_status == "approved") {
    return db.query(
      `UPDATE request SET request_status = 'canceled' WHERE id = ?`,
      [request_id],
      callback
    );
  } else if (request_status == "paid") {
    const trip = await util.promisifyQuery(
      `SELECT request.trip_id FROM request WHERE id = ?`,
      [request_id]
    );
...
const requestApprove = (ID, callback) => {
  return db.query(
    `UPDATE request SET request_status = 'approved' WHERE id = ?`,
    [ID],
    callback
  );
};
...
const requestReject = (ID, callback) => {
  return db.query(
    `UPDATE request SET request_status = 'rejected' WHERE id = ?`,
    [ID],
    callback
  );
};
...
const getPendingRequest = (ID, callback) => {
  return db.query(
    `SELECT 
                            ...
                            members.photo
                            FROM request left join members on request.member_id = members.id 
                            WHERE request.trip_id = ? AND request.request_status = "pending"`,
    [ID],
    callback
  );
};
...
const getPassengerByTripId = (trip_id, callback) => {
  return db.query(
    `SELECT
                            ...
                            WHERE request.request_status IN ('approved','paid','on going','done') AND request.trip_id =` +
      trip_id,
    callback
  );
};
...
const getAllPassengerForDriver = (trip_id, callback) => {
  return db.query(
    `SELECT 
                            ...
                            WHERE request.request_status IN ('approved','paid','on going','done') AND trip.id = ? `,
    [trip_id],
    callback
  );
};
