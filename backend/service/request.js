...
function createRequest(trip_id,member_id,data,created_at,callback) {
    ...
    const reqStatus = 2;
    return db.query(`INSERT INTO request (member_id,trip_id,departure_latitude,departure_longtitude,departure_detail,destination_latitude,destination_longtitude,destination_detail,status,created_at) 
    values (?,?,?,?,?,?,?,?,?,?)`,
...
function getRequestInfo(request_id,callback) {
    ...
    status,review_id,departed_at,driver_departed_at,driver_arrived_at,created_at,paid_at
    WHERE id = ?`
    , [request_id], callback); 
};
